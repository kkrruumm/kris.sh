---
title: "What Linux distro should I use?"
...
Everything sucks in one way or another? (foreshadowing) Well, you could always create your own or find something like [kiss linux](https://kisscommunity.bvnf.space/) to base off of and bootstrap your own thing, though I do think this is (slightly) unreasonable to expect from a new user.
...
In the context of distributions that are typically recommended to new users, [Linux Mint](https://linuxmint.com/) for example is one that usually comes up in the discussion, and with no hostility towards Linux Mint whatsoever, there is no stable/established Wayland support with any of their default desktop environment options.
...
I think there's a happy medium here- my personal general distribution of choice, [Void](https://voidlinux.org/), *typically* does not roll out "bleeding edge" software, but is also not generally "outdated." I tend to also be a fan of distributions that roll out a new release every 6 months or so, an example of a distro that does this (or sometimes, roughly this) would be [Fedora](https://fedoraproject.org/), though for various reasons (moderately annoying to pull Nvidia graphics drivers, weirdness around codecs, heavy-handed corporate involvement regardless of what people may suggest), I find it difficult to recommend. [Alpine](https://alpinelinux.org/) is another distribution that roughly does this, though quite difficult to recommend to newer users.

[Arch](https://archlinux.org/) would be a distro with **relatively** sanely up to date software, though I am personally not a fan of the "update just because there's an update" mentality around bleeding edge distributions. 

# Software availability

While less of a concern compared to the days of old due to awesome solutions like [flatpak](https://flatpak.org/), software availability does matter in certain contexts. I imagine most users are allergic to packaging the things they need themselves, and should find something that has a lot of packages (preferably of decent quality) available.
...
Arch is pretty good at this due to the [AUR](https://aur.archlinux.org/), or Arch User Repository. The problem here is that random things on the AUR **cannot** be trusted. At minimum, you should be seeking packages that have a positive reputation and good comments, and preferably actually auditing packages you install yourself. There have been **many** cases of something malicious being uploaded to the AUR, and really the responsibility for that falls on the user being conscious of what they're doing.
...
1. [Void](https://voidlinux.org/) - Has been my daily driver on both server and desktop for nearing 4 years as of the time of writing.

2. [Kiss](https://kisscommunity.bvnf.space/) - Uses a software stack I agree with by default assuming one uses the musl branch.

3. [Crux](https://crux.nu/) - I like the packaging system, and may only get slightly irritated with glibc.
...
Depending on someones level of experience with computers in general, I have a knack for recommending Arch installed via [archinstall](https://wiki.archlinux.org/title/Archinstall), as one of these "shots in the dark."
